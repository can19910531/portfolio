...
4. 平時開試算表「批次退款總表」分頁看即時進度；要轉帳那天按「📋 匯出批次退款表」拿 Excel 定版：已填帳號的直接帶入、轉購物金的標綠字、沒填的標紅字
...
1. **建 Google 試算表**：建立試算表（名稱自訂），三個分頁：
   ...
   - `退款帳號回覆`：表頭 `送出時間 會員編號 姓名 應退金額 銀行代碼 帳號 狀態 類型`
   - （選用）`批次退款總表`：即時進度看板，A2 放一條公式把「應退名單 × 最新回覆」連動起來（公式見主理人版倉庫，或請 AI 依上述兩分頁欄位寫一條 FILTER+VLOOKUP+SORT）
